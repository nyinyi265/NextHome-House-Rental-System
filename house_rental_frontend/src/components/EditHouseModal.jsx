...
export default function EditHouseModal({ isOpen, onClose, onSuccess, token, house }) {
  ...
  // Initialize form data when house is passed
  useEffect(() => {
    if (house && isOpen) {
      setFormData({
        title: house.title || '',
        slug: house.slug || '',
        type: house.property_type || house.type || 'apartment',
        ...
        price: house.price_per_month?.toString() || house.price?.toString() || '',
        is_available: house.is_available ?? true,
        available_from: house.available_from || '',
      });

      // Set selected amenities
      const houseAmenities = house.amenties || house.amenities || [];
      setSelectedAmenities(houseAmenities.map(a => a.id || a));

      // Set selected furnitures
      const houseFurnitures = house.furnitures || [];
      setSelectedFurnitures(houseFurnitures.map(f => f.id || f));

      // Set existing photos
      const photos = house.house_photos || house.photos || [];
      setExistingPhotos(photos);

      // Reset new photos and delete tracking
      setNewPhotos([]);
      setPhotosToDelete([]);
    }
  }, [house, isOpen]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await houseService.updateHouse(token, house.id, data);
      onSuccess();
      handleClose();
    } catch (err) {
      console.error('Failed to update house', err);
      setError(err.message || err.error || 'Failed to update house. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          ...
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {existingPhotos.map((photo) => (
                    <div key={photo.id} className="relative group">
                      <img
                        src={env.getImageUrl(photo.photo_path)}
                        alt="Property photo"
            ...
            {/* Photos - Add New */}
            <div>
              <h3 className="text-lg font-semibold mb-4">Add New Photos</h3>
